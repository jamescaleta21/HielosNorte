...
Dim PSFAR_TRANS1 As rdoQuery
Dim FAR_TRANS1 As rdoResultset
...
Dim WS_NOMVEN As String * 20
Dim WS_NOMBAN As String * 20
Dim WS_SALDO As Currency
Dim WS_MONEDA As String * 1
Dim ww_moneda
Dim ws_mensaje
Dim WS_FBG, WS_LARGO, ws_nomche, ws_codclie, ws_codven
...
Do Until all_llave.EOF
   ...
   ws_codven = Val(all_llave!ALL_CODVEN)
   If all_llave!all_numfac <> 0 Then
       ...
       ElseIf all_llave!ALL_FBG = "P" Then
          WS_FBG = "Ped." & all_llave!ALL_NUMSER & "-" & all_llave!all_numfac
       Else
   ...
   If ws_codven <> 0 Then
         ...
         SQ_OPER = 1
         LEER_VEN_LLAVE
         WS_NOMVEN = Left(ven_llave!VEM_NOMBRE, 18) & ":"
   End If
   ...
   End If
   WS_LARGO = Trim(WS_NOMCLI) & " " & Trim(WS_FBG) & " " & Trim(WS_NOMBAN) & Trim(ws_nomche) & " " & all_llave!all_concepto
...
Do Until all_llave.EOF
   ...
   If LK_EMP = "HER" And all_llave!ALL_CODTRA = 2727 And all_llave!ALL_SIGNO_CAR = 1 Then GoTo OTRO
   If all_llave!all_flag_ext = "E" Then GoTo OTRO
   If (all_llave!ALL_SIGNO_CAR = 0 And all_llave!ALL_tipmov = 0) Or (all_llave("all_codtra") = 2725 Or all_llave("all_codtra") = 5360 Or all_llave("all_codtra") = 2770 Or all_llave("all_codtra") = 2774) Then
      WS_IMPORTE = all_llave!ALL_IMPORTE
   Else
      WS_IMPORTE = all_llave!ALL_IMPORTE_AMORT
   End If
...
'   End If
   ...
   ws_nomche = ""
   ws_codclie = Val(all_llave!ALL_CODCLIE)
   If all_llave!all_numfac <> 0 And all_llave!ALL_CP = "C" Then
       If all_llave!ALL_FBG = "F" Then
          WS_FBG = "Fact. " & all_llave!ALL_NUMSER & "-" & all_llave!all_numfac
       ElseIf all_llave!ALL_FBG = "B" Then
          WS_FBG = "Bolet." & all_llave!ALL_NUMSER & "-" & all_llave!all_numfac
   ...
   If all_llave!all_numfac <> 0 And all_llave!ALL_CP = "P" Then
       If all_llave!ALL_FBG = "F" Then
          WS_FBG = "Fact. " & all_llave!all_numser_c & "-" & all_llave!all_numfac_c
       ElseIf all_llave!ALL_FBG = "B" Then
          WS_FBG = "Bolet." & all_llave!all_numser_c & "-" & all_llave!all_numfac_c
       Else
          WS_FBG = "Factura: " & all_llave!all_numser_c & "-" & all_llave!all_numfac_c
       End If
   End If
...
'   End If
   ...
   xl.Cells(f1, 2) = all_llave!ALL_FECHA_DIA
    If Trim(all_llave!ALL_autocon) = "" Then
        xl.Cells(f1, 3) = WS_LARGO + " " + all_llave!all_concepto 'WS_LARGO
    Else
        xl.Cells(f1, 3) = WS_LARGO + " " + all_llave!all_concepto 'WS_LARGO
    End If
   xl.Cells(f1, 6) = WS_SALDO
...
If CDate(wsFECHA1) > CDate(wsFECHA2) Then
 ...
 GoTo CANCELA
End If
'If CDate(wsFECHA1) <> cop_llave!cop_fecha_proceso Then cheasiento.Value = 0  QUITADO GTS
'If CDate(wsFECHA2) <> cop_llave!cop_fecha_proceso2 Then cheasiento.Value = 0   QUITADO GTS
GoSub WEXCEL
If LK_EMP = "PIU" Then
  max.Text = 90000
End If
...
If WCONTROL = 1 Then ' Compras(20) y Provisiones (99)
  If Val(txt_cli.Text) <> 0 Then
   ...
   pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCLIE = " & Trim(txt_cli.Text) & " AND FAR_CODCIA = ? AND (FAR_TIPMOV = 20 OR FAR_TIPMOV = 99) AND FAR_ESTADO = 'N' AND FAR_FECHA_PRO >= ? AND FAR_FECHA_PRO <= ? AND FAR_CP = 'P'  ORDER BY FAR_FBG,FAR_NUMFAC,FAR_FECHA_PRO, FAR_NUMOPER"
  Else
   pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCIA = ? AND (FAR_TIPMOV = 20 OR FAR_TIPMOV = 99) AND FAR_ESTADO = 'N' AND FAR_FECHA_PRO >= ? AND FAR_FECHA_PRO <= ? AND FAR_CP = 'P'  ORDER BY FAR_FBG,FAR_NUMFAC,FAR_FECHA_PRO, FAR_NUMOPER"
  End If
  WS_SIGNO = 1
ElseIf WCONTROL = 2 Then ' Notas de Crédito
  If Val(txt_cli.Text) <> 0 Then
    ...
    pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCLIE = " & Trim(txt_cli.Text) & " AND FAR_CODCIA = ? AND FAR_TIPMOV = 97 AND FAR_ESTADO = 'N' AND FAR_FECHA_PRO >= ? AND FAR_FECHA_PRO <= ? AND FAR_FBG = 'C'  AND FAR_CP = 'P'  ORDER BY FAR_NUMFAC,FAR_FECHA_PRO, FAR_NUMOPER"
  Else
    pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCIA = ? AND FAR_TIPMOV = 97 AND FAR_ESTADO = 'N' AND FAR_FECHA_PRO >= ? AND FAR_FECHA_PRO <= ? AND FAR_FBG = 'C'  AND FAR_CP = 'P'  ORDER BY FAR_NUMFAC,FAR_FECHA_PRO, FAR_NUMOPER"
  End If
  WS_SIGNO = -1
ElseIf WCONTROL = 3 Then
  ...
  If Val(txt_cli.Text) <> 0 Then
    pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCLIE = " & Trim(txt_cli.Text) & " AND FAR_CODCIA = ? AND FAR_TIPMOV = 98 AND FAR_ESTADO = 'N' AND FAR_FECHA_PRO >= ? AND FAR_FECHA_PRO <= ? AND FAR_FBG = 'A' AND FAR_ESTADO <> 'E' AND FAR_CP = 'P'  ORDER BY FAR_NUMFAC,FAR_FECHA_PRO, FAR_NUMOPER"
  Else
    pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCIA = ? AND FAR_TIPMOV = 98 AND FAR_FECHA_PRO >= ? AND FAR_FECHA_PRO <= ? AND FAR_FBG = 'A' AND FAR_ESTADO = 'N' AND FAR_CP = 'P'  ORDER BY FAR_NUMFAC,FAR_FECHA_PRO, FAR_NUMOPER"
  End If
  ' Fin Agregado
  WS_SIGNO = 1
End If
...
'  xl.Application.Visible = True
  ...
  wq_codven = llave_rep02!FAR_CODVEN
  www_fbg = Trim(llave_rep02!far_fbg)
  wq_fbg = Trim(llave_rep02!far_cod_sunat) ' Trim(llave_rep02!far_fbg)
  wq_docu = "'" & llave_rep02!far_numfac
  ...
  If wq_fbg = "01" Or wq_fbg = "1" Or wq_fbg = "14" Or wq_fbg = "7" Or wq_fbg = "8" Or wq_fbg = "55" Or wq_fbg = "99" Or wq_fbg = "12" Then
  ...
  Else   'aca no disgrega el IGV gts
  WQ_IMPTO = (Format(Val(llave_rep02!far_IMPTO), "0.000") * WS_SIGNO) * AWQ_TIPO_CAMBIO
  WQ_TOTAL = (Val(llave_rep02!FAR_BRUTO) + Val(llave_rep02!far_IMPTO) - Val(llave_rep02!FAR_TOT_DESCTO) + Val(llave_rep02!FAR_GASTOS)) * WS_SIGNO
  'WQ_TOTAL = wq_bruto + wq_gastos + WQ_IMPTO '(wq_total * AWQ_TIPO_CAMBIO)
  
  wq_bruto = WQ_TOTAL '(Val(llave_rep02!FAR_BRUTO) - Val(llave_rep02!FAR_TOT_DESCTO)) * WS_SIGNO
  wq_bruto = Format(WQ_TOTAL * AWQ_TIPO_CAMBIO, "0.000")
  wq_desto = (Val(llave_rep02!FAR_TOT_DESCTO) * WS_SIGNO) * AWQ_TIPO_CAMBIO
  wq_gastos = (Val(llave_rep02!FAR_GASTOS) * WS_SIGNO) * AWQ_TIPO_CAMBIO
  wq_flete = (Val(Nulo_Valor0(llave_rep02!FAR_TOT_FLETE)) * WS_SIGNO) * AWQ_TIPO_CAMBIO
  'wq_tot_descto =
  WQ_IMPTO = 0 '(Format(Val(llave_rep02!far_IMPTO), "0.000") * WS_SIGNO) * AWQ_TIPO_CAMBIO
  WQ_TOTAL = WQ_TOTAL '(Val(llave_rep02!FAR_BRUTO) + Val(llave_rep02!far_IMPTO) - Val(llave_rep02!FAR_TOT_DESCTO) + Val(llave_rep02!FAR_GASTOS)) * WS_SIGNO
  WQ_TOTAL = WQ_TOTAL 'wq_bruto + wq_gastos + WQ_IMPTO '(wq_total * AWQ_TIPO_CAMBIO)
  End If
  If wq_bruto = 0 Then WQ_TOTAL = 0
  wq_estado = llave_rep02!far_estado
  If wq_estado <> "E" Then
...
Dim wSTOCKACTUAL As Currency
Dim WINGRESOS As Currency
Dim wSALIDAS As Currency
...
pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCIA = ? AND FAR_FECHA_COMPRA >= ? AND FAR_CODART = ? and far_estado<>'E' and far_transito <> 'P' ORDER BY FAR_FECHA_COMPRA desc, FAR_SIGNO_ARM, FAR_NUMOPER DESC"
Set PS_REP01 = CN.CreateQuery("", pub_cadena)
PS_REP01(0) = 0
PS_REP01(1) = LK_FECHA_DIA
PS_REP01(2) = 0
...
'pub_cadena = "SELECT * FROM FACART WHERE FAR_CODCIA = ? AND FAR_FECHA_COMPRA >= ? AND FAR_CODART = ? and far_estado<>'E' and far_transito <> 'P' and far-signo_arm = -1 ORDER BY FAR_FECHA_COMPRA desc, FAR_SIGNO_ARM, FAR_NUMOPER DESC"
'Set PS_REP03 = CN.CreateQuery("", pub_cadena)
'PS_REP03(0) = 0
'PS_REP03(1) = LK_FECHA_DIA
'PS_REP03(2) = 0
...
Do Until llave_rep02.EOF
       ...
       ' If llave_rep02!ART_KEY = 2977 Then Stop
...
'             GoTo CANCELA
     ...
     PUB_CADENA1 = "SELECT isnull(sum(far_cantidad),0) FROM FACART " & _
        "WHERE  FAR_CODCIA = ? AND FAR_FECHA_COMPRA >= ? AND " & _
        "far_codart= ?   AND " & _
        "FAR_SIGNO_ARM = -1 AND FAR_TRANSITO <> 'P' AND FAR_ESTADO <> 'E'"
        
        Set PSFAR_TRANS1 = CN.CreateQuery("", PUB_CADENA1)
        PSFAR_TRANS1.rdoParameters(0) = LK_CODCIA
        PSFAR_TRANS1.rdoParameters(1) = wsFECHA1
        PSFAR_TRANS1.rdoParameters(2) = PUB_CODART
        Set FAR_TRANS1 = PSFAR_TRANS1.OpenResultset(rdOpenKeyset, rdConcurValues)
        
     PUB_CADENA2 = "SELECT isnull(sum(far_cantidad),0) FROM FACART " & _
        "WHERE  FAR_CODCIA = ? AND FAR_FECHA_COMPRA >= ? AND " & _
        "far_codart= ?   AND " & _
        "FAR_SIGNO_ARM = 1 AND FAR_TRANSITO <> 'P' AND FAR_ESTADO <> 'E'"
        
        Set PSFAR_TRANS2 = CN.CreateQuery("", PUB_CADENA2)
        PSFAR_TRANS2.rdoParameters(0) = LK_CODCIA
        PSFAR_TRANS2.rdoParameters(1) = wsFECHA1
        PSFAR_TRANS2.rdoParameters(2) = PUB_CODART
    ...
    wSTOCK = Val(arm_llave!ARM_STOCK)
    wSALIDAS = Val(FAR_TRANS1(0))
    WINGRESOS = Val(FAR_TRANS2(0))
    PUB_CODART = WSCODART
    pu_codcia = LK_CODCIA
    SQ_OPER = 1
    SQ_OPER = 1
    ...
    xl.Cells(f1, 5) = Val(wSALIDAS)
    xl.Cells(f1, 6) = Val(WINGRESOS)
    xl.Cells(f1, 7) = Val(wSTOCK) + Val(wSALIDAS) - Val(WINGRESOS)
   ' xl.Cells(f1, 8) = Val(WD_COSPRO)
   ' xl.Cells(f1, 9) = Format(Val(WD_COSPRO) * Val(xl.Cells(f1, 4)), "0.00")
