...
Public PUB_NOMCHOFER As String * 80
...
Dim PUB_CADENA1 As String '--JC
Dim PSFAR_TRANS1 As rdoQuery '--JC
Dim FAR_TRANS1 As rdoResultset '--JC
Dim PUB_CADENA2 As String '--JC
Dim PSFAR_TRANS2 As rdoQuery '--JC
Dim FAR_TRANS2 As rdoResultset '--JC
Dim vfarcantidad As Integer  '--JC
...
ElseIf numarchi = 0 Then
 LV1.ColumnHeaders.Add 1, , "Descripción", 4500
 LV1.ColumnHeaders.Add 2, , "Cod.", 0
 LV1.ColumnHeaders.Add 3, , "Sub-Codigo", 600
 LV1.ColumnHeaders.Add 4, , "Division", 2200
 LV1.ColumnHeaders.Add 5, , "Familia", 2200
 LV1.ColumnHeaders.Add 6, , "Stock Fisico", 1500
 'adiciono campos Cantidad por Despachar y Diferencia(Stock Real)--JC
 LV1.ColumnHeaders.Add 7, , "Por Despachar", 0
 LV1.ColumnHeaders.Add 8, , "Saldo", 0
 LV1.ColumnHeaders.Add 9, , "Por Recepcionar", 0
Else
...
'        "WHERE far_codart=? and FAR_CP = 'C' AND FAR_CODCIA=? AND " & _
'        "FAR_TIPMOV ='10' AND far_transito='P' AND FAR_ESTADO <> 'E'"
'
'        Set PSFAR_TRANS1 = CN.CreateQuery("", PUB_CADENA1)
'        PSFAR_TRANS1.rdoParameters(0) = X.rdoColumns(0)
'        PSFAR_TRANS1.rdoParameters(1) = PUB_CODCIA
'        Set FAR_TRANS1 = PSFAR_TRANS1.OpenResultset(rdOpenKeyset, rdConcurValues)
'        vfarcantidad = FAR_TRANS1(0)
'        '********************Ahora suma pero de movimiento 25**************
'         PUB_CADENA1 = "SELECT isnull(sum(far_cantidad),0) FROM FACART " & _
'        "WHERE far_codart=? and FAR_CP = 'C' AND FAR_CODCIA=? AND " & _
...
'        PSFAR_TRANS1.rdoParameters(1) = PUB_CODCIA
'        Set FAR_TRANS1 = PSFAR_TRANS1.OpenResultset(rdOpenKeyset, rdConcurValues)
'        itmX.SubItems(6) = Format((vfarcantidad - FAR_TRANS1(0)), "0.000")
'        'itmX.SubItems(6) = Format(FAR_TRANS1(0), "0.000")
'        itmX.SubItems(7) = Format(Val(itmX.SubItems(5)) - Val(itmX.SubItems(6)), "0.000")
'
'
''        '*********DANDO LOS DATOS
'        PUB_CADENA2 = "SELECT isnull(sum(far_cantidad),0) FROM FACART " & _
'        "WHERE far_codart=? and FAR_CP = 'P' AND FAR_CODCIA=? AND " & _
'        "FAR_TIPMOV ='20' AND far_transito='P'"
'
'        Set PSFAR_TRANS2 = CN.CreateQuery("", PUB_CADENA2)
'        PSFAR_TRANS2.rdoParameters(0) = X.rdoColumns(0)
'        PSFAR_TRANS2.rdoParameters(1) = PUB_CODCIA
'        Set FAR_TRANS2 = PSFAR_TRANS2.OpenResultset(rdOpenKeyset, rdConcurValues)
'        vfarcantidad = FAR_TRANS2(0)
'        '********************Ahora suma pero de movimiento 101**************
'         PUB_CADENA2 = "SELECT isnull(sum(far_cantidad),0) FROM FACART " & _
'        "WHERE far_codart=? and FAR_CP = 'P' AND FAR_CODCIA=? AND " & _
...
'        PSFAR_TRANS2.rdoParameters(1) = PUB_CODCIA
'        Set FAR_TRANS2 = PSFAR_TRANS2.OpenResultset(rdOpenKeyset, rdConcurValues)
'        itmX.SubItems(8) = Format((vfarcantidad - FAR_TRANS2(0)), "0.000")
'        'itmX.SubItems(6) = Format(FAR_TRANS1(0), "0.000")
'        itmX.SubItems(7) = Format(Val(itmX.SubItems(5)) - Val(itmX.SubItems(8)), "0.000")
'
      End If
   End If
...
SQ_OPER = 2
PUB_CODCIA = LK_CODCIA
PUB_CODVEN = 1 'Val(FORMGEN.i_codven.Text)
LEER_PAR_LLAVE
If pac_llave.EOF Then
...
If llave_rep01!REP_CODTRA = 2401 Or llave_rep01!REP_CODTRA = 2414 Or llave_rep01!REP_CODTRA = 1405 Or llave_rep01!REP_CODTRA = 1450 Then
    ...
    If llave_rep01!REP_CODTRA = 1405 Then
    FORMGEN.Reportes.Formulas(1) = "SON=  '" & CONVER_LETRAS(PUB_NETO, WMONE) & "'"
      pub_cadena = "{FACART.FAR_ESTADO} <> 'E' AND {FACART.FAR_TIPMOV} = 25 and {FACART.FAR_CODCIA} = '" & LK_CODCIA & "'  AND {FACART.FAR_NUMSER}= '" & PU_NUMSER & "' AND {FACART.FAR_NUMFAC} = " & PU_NUMFAC
      'pub_cadena = "{FACART.FAR_ESTADO} <> 'E' AND {FACART.FAR_TIPMOV} = 25 and {FACART.FAR_CODCIA} = '" & LK_CODCIA & "' AND {FACART.FAR_NUMFAC} = " & PU_NUMFAC
    FORMGEN.Reportes.ReportFileName = wRuta & "GRE006new.RPT"
      GoTo pasa_todo
    End If
    ...
    ElseIf Trim(FORMGEN.i_fbg.Text) = "P" Then
      ...
      FORMGEN.Reportes.ReportFileName = wRuta & Trim(pac_llave!PAC_ARCHI_G)
    End If
...
If tab_mayor.EOF Then
  MsgBox "Crear tab_tipreg = 155 para seguridas de meses.", 48, Pub_Titulo
  CHE_BLOQ_MES = 1
Else
    Do Until tab_mayor.EOF
