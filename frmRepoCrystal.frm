...
If ListView1.Visible = False And KeyCode <> 13 Or Len(i_codart2.Text) = 1 Then
    ...
    archi = archi & "WHERE (TABLAS.TAB_TIPREG = 122) AND (TABLAS_1.TAB_TIPREG = 123) AND (PRECIOS.PRE_FLAG_UNIDAD = 'A') AND ARTI.ART_CODCIA = '" & LK_CODCIA & "' AND ARTI.ART_CALIDAD = 1 AND ARTI.ART_FLAG_STOCK = 'M' AND ARTI.ART_NOMBRE BETWEEN '" & Trim(i_codart2.Text) & "%' AND  '" & var & "' ORDER BY ARTI.ART_NOMBRE"
    End If
...
Dim TOTAL_COMPRAS As Currency
Dim TOTAL_COMPRAS_SOLES As Currency
Dim TOTAL_VENTAS As Currency
Dim TOTAL_VENTAS_SOLES As Currency
...
'                 xl.Cells(f1, 2) = "Total Division: "
'                 xl.Cells(f1, 11) = q_stock
'                 xl.Cells(f1, 12) = q_stock_val
'                 xl.Cells(f1, 9) = acu_val_salidas / ((100 + LK_IGV) / 100)
'                 xl.Cells(f1, 6) = acu_val_ingresos
...
'                End If
        ...
        PUB_SECUEN = 0
        PUB_CODART = PUB_KEY
        'If PUB_CODART = 7263 Then Stop
        LEER_PRE_LLAVE
        f1 = f1 + 1
        xl.Cells(f1, 1) = llave_rep02!art_alterno '& " " & ww_codcia
        xl.Cells(f1, 2) = Trim(PUB_CONCEPTO)
        xl.Cells(f1, 8) = Trim(pre_llave!pre_unidad)
        f1 = f1 + 1
        xl.Cells(f1, 3) = "Saldo Inicial "
        xl.Cells(f1, 10) = Format(WCOSPRO_SUP, "0.00") 'Val(llave_rep01!FAR_COSPRO) 'Format(WCOSPRO_SUP, "0.00") '
'        PUB_IMPORTE = (llave_rep01!FAR_STOCK + ((llave_rep01!far_SIGNO_aRM * llave_rep01!FAR_CANTIDAD) * -1))
        ...
        xl.Cells(f1, 11) = PUB_IMPORTE
        PUB_IMPORTE_AMORT = PUB_IMPORTE * Format(WCOSPRO_SUP, "0.00") 'Val(llave_rep01!FAR_COSPRO)
...
'           xl.Application.Visible = True
           ...
           If llave_rep01!far_signo_arm = 1 Then
              ...
              xl.Cells(f1, 5) = WS_PRECIO
              xl.Cells(f1, 6) = Val(ww_ingreso)
              CHE_KARDEX = CHE_KARDEX + WCAT
              
           End If
           If llave_rep01!far_signo_arm = -1 Then
              ww_salida = WCAT * WS_PRECIO
              acu_val_salidas = acu_val_salidas + ww_salida
              xl.Cells(f1, 7) = WCAT
              xx_salida = xx_salida + WCAT
              xl.Cells(f1, 8) = WS_PRECIO / ((100 + LK_IGV) / 100)
              xl.Cells(f1, 9) = Val(ww_salida) / ((100 + LK_IGV) / 100)
              CHE_KARDEX = CHE_KARDEX - WCAT
           End If
...
'           If Val(PUB_IMPORTE) = 0 Then Stop
           xl.Cells(f1, 11) = Val(PUB_IMPORTE)
           xl.Cells(f1, 12) = Val(PUB_IMPORTE_AMORT)
       ...
       'xl.Application.Visible = True
        ...
        xl.Cells(f1, 11) = PUB_IMPORTE
        xl.Cells(f1, 10) = Val(llave_rep02!arm_cospro)
        xl.Cells(f1, 12) = PUB_IMPORTE * Val(llave_rep02!arm_cospro)
        ...
        TOTAL_COMPRAS = TOTAL_COMPRAS + xx_ingreso
        TOTAL_COMPRAS_SOLES = TOTAL_COMPRAS_SOLES + acu_val_ingresos
        TOTAL_VENTAS = TOTAL_VENTAS + xx_salida
        TOTAL_VENTAS_SOLES = TOTAL_VENTAS_SOLES + acu_val_salidas / ((100 + LK_IGV) / 100)
...
'   xl.Worksheets(1).Rows(f1).RowHeight = 12
   ...
   xl.Cells(f1, 4) = Format(TOTAL_COMPRAS, "#,##0.00")
   xl.Cells(f1, 6) = Format(TOTAL_COMPRAS_SOLES, "#,##0.00")
   xl.Cells(f1, 7) = Format(TOTAL_VENTAS, "#,##0.00")
   xl.Cells(f1, 9) = Format(TOTAL_VENTAS_SOLES, "#,##0.00")
   xl.Cells(f1, 11) = Format(TOTAL_CLASE, "#,##0.00")
   xl.Cells(f1, 12) = Format(TOTAL_CLASE_VAL, "#,##0.00")
