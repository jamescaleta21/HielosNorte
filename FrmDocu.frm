...
End If
SIN_CODART = 0
If LOC_TIPMOV = 75 Or LOC_TIPMOV = 99 Or LOC_TIPMOV = 102 Or LOC_TIPMOV = 25 Or LOC_TIPMOV = 3 Or LOC_TIPMOV = 100 Or LOC_TIPMOV = 101 Or LOC_TIPMOV = 93 Or LOC_TIPMOV = 20 Or LOC_TIPMOV = 5 Or LOC_TIPMOV = 6 Or LOC_TIPMOV = 10 Or LOC_TIPMOV = 97 Or LOC_TIPMOV = 98 Then
...
'Unload FORM_GRIFO
'*********************************************************
SQL = "SELECT DIRCOMP,DIREC FROM DIRCLI " & _
...
loc_flag_espera = ""
LOC_ARROZ = ""
pub_cadena = "SELECT * FROM TRANSPORTE WHERE TRN_KEY = ? AND TRN_CODCIA = ? ORDER BY TRN_KEY"
...
LOC_NUMFAC_FIN = 0
pub_cadena = "SELECT * FROM TRANSPORTE WHERE TRN_KEY >= ? AND TRN_CODCIA = ? ORDER BY TRN_NOMBRE"
Set pstransporte = CN.CreateQuery("", pub_cadena)
pstransporte.rdoParameters(0) = 0
pstransporte.rdoParameters(1) = LK_CODCIA
...
Do Until TRANSPORTE.EOF
    TRANS.AddItem Trim(TRANSPORTE!TRN_NOMBRE) & String(40, " ") & TRANSPORTE!TRN_KEY
    TRANSPORTE.MoveNext
Loop
TRANS.Enabled = False
pub_cadena = "SELECT * FROM facart WHERE FAR_TIPMOV = ? AND FAR_CODCIA = ? AND FAR_NUMSER = ? AND FAR_FBG=? AND FAR_NUMFAC = ?    ORDER BY FAR_TIPMOV, FAR_CODCIA, FAR_NUMSER, FAR_FBG, FAR_NUMFAC, FAR_NUMSEC"
...
If Trim(cli_llave!cli_codclie) = 1 Then
   d_nomclie.Caption = Trim(cli_llave!CLI_NOMBRE)    ' GTS p Dirome cambio Trim(far_r!FAR_CLIENTE) por (cli_llave!CLI_NOMBRE)
Else
...
'comienza llenado
  ...
  Do Until rs.EOF
   ...
   rs.MoveNext
  Loop
  txtdire.Text = d_dire  'agregado gts
...
'End If
    ...
    Reportes.Formulas(29) = ""
    Reportes.Formulas(30) = ""
    Reportes.Formulas(31) = ""
    Reportes.Formulas(32) = ""
    Reportes.Formulas(33) = ""
    Reportes.Formulas(34) = ""

    
    
    If LOC_TIPMOV = 3 Or LOC_TIPMOV = 25 Or LOC_TIPMOV = 6 Or LOC_TIPMOV = 100 Or LOC_TIPMOV = 101 Then
        frmdocu.Reportes.WindowTitle = "GUIA DE REMISION  :" & Format(PU_NUMSER, "000") & " - " & Format(PU_NUMFAC, "0000000")
        If LOC_TIPMOV = 25 Or LOC_TIPMOV = 100 Or LOC_TIPMOV = 101 Then
    ...
    If LOC_TIPMOV = 10 Or LOC_TIPMOV = 97 Or LOC_TIPMOV = 98 Or LOC_TIPMOV = 25 Then
      ...
      ElseIf Left(frmdocu.cmbFBG.Text, 1) = "G" Then
        ...
        frmdocu.Reportes.ReportFileName = wRuta + "GRE006.RPT"
      ElseIf Left(frmdocu.cmbFBG.Text, 1) = "P" Then
        frmdocu.Reportes.WindowTitle = " PEDIDO  : " & Format(PU_NUMSER, "000") & " - " & Format(PU_NUMFAC, "0000000") & " al " & Format(LOC_NUMFAC_FIN, "0000000")
    ...
    Else
       pub_cadena = "{FACART.FAR_ESTADO} <> 'E' AND ({FACART.FAR_TIPMOV} = 10 OR {FACART.FAR_TIPMOV} = 25)  AND {FACART.FAR_CODCIA} = '" & LK_CODCIA & "' AND {FACART.FAR_FBG} = '" & Left(frmdocu.cmbFBG.Text, 1) & "' AND {FACART.FAR_NUMSER}= '" & wser & "' AND ( {FACART.FAR_NUMFAC} >= " & PU_NUMFAC & " and {FACART.FAR_NUMFAC} <= " & LOC_NUMFAC_FIN & ")"
     End If
    Reportes.Formulas(12) = ""
    Reportes.Formulas(13) = ""
    Reportes.Formulas(14) = ""
...
PASA_OP:
       If LOC_TIPMOV = 10 Or LOC_TIPMOV = 100 Or LOC_TIPMOV = 3 Or LOC_TIPMOV = 25 Then
         ...
         Else
             If LOC_TIPMOV = 25 Then   'GTS
             frmdocu.Reportes.ReportFileName = wRuta + "GRE006NEW.RPT"
             Else
             frmdocu.Reportes.ReportFileName = wRuta + Trim(pac_llave!PAC_ARCHI_GUIA) ' "FACGUIA.RPT"
             End If
         End If
         ...
         If Pub_Respuesta = vbYes Then
           ...
           Reportes.Formulas(15) = ""
           Reportes.Formulas(16) = ""
           Reportes.Formulas(29) = "DIRECCION='" & d_domicilio.Caption & "'"
           Reportes.Formulas(30) = "VIA='" & txtvia.Text & "'"
           Reportes.Formulas(31) = "NRO='" & txtnro.Text & "'"
           Reportes.Formulas(32) = "DISTRITO='" & txtdistrito.Text & "'"
           Reportes.Formulas(33) = "PROVINCIA='" & txtprovincia.Text & "'"
           Reportes.Formulas(34) = "DEPA='" & txtdepa.Text & "'"
           ...
           If chetrans.Value = 1 Then
             ...
             Reportes.Formulas(15) = "TRN_DNI       =  '" & llave_trans!TRN_DNI & "'"
             Reportes.Formulas(16) = "TRN_PLACA     =  '" & llave_trans!TRN_PLACA & "'"
'             If LK_EMP = "PIU" Or LK_EMP = "3AA" Then   QUITADO GTS
              ...
              Reportes.Formulas(21) = "TRN_CERT =  '" & llave_trans!TRN_CERT & "'"
'             End If
           End If
           frmdocu.Reportes.WindowTitle = frmdocu.Reportes.WindowTitle & " Archivo: " & Trim(frmdocu.Reportes.ReportFileName)
...
Private Sub CrearArchivoPlano2(cTipoDocto As String, cSerie As String, cNumero As Double)
   ...
   ' End If
    ...
    End If
    ElseIf LK_CODCIA = "05" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "06" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "07" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "08" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "09" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "10" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "11" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "12" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "13" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "14" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "15" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "16" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "17" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "18" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "19" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "20" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "21" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "22" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "23" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "24" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "25" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "26" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "27" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "28" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "29" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    ElseIf LK_CODCIA = "30" Then
    Set ArchivoCab = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOcab, True)
    Set ArchivoTri = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOtri, True)
    Set ArchivoLey = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOley, True)
    Set ArchivoAca = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOaca, True)
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
    Set ArchivoPAG = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOpag, True)
    End If
    Else
    End If
    ...
    ElseIf LK_CODCIA = "05" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "06" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "07" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "08" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "09" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "10" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "11" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "12" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "13" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "14" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "15" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "16" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "17" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "18" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "19" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "20" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "21" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "22" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "23" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "24" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "25" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "26" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "27" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "28" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "29" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    ElseIf LK_CODCIA = "30" Then
    Set ArchivoDet = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOdet, True)
    Else
    End If
    End If
    ...
    Do While Not orsLey.EOF
        sCadena = sCadena & orsLey!cod & "|" & Trim(CONVER_LETRAS(Me.d_neto.Caption, "S")) & "|"
    ...
    If cTipoDocto = "F" And LOC_TIPMOV = 10 Then
            ...
            ElseIf LK_CODCIA = "02" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config2.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "03" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config3.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "04" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config4.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "05" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config5.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "06" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config6.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "07" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config7.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "08" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config8.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "09" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config9.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "10" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config10.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "11" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config11.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "12" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config12.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "13" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config13.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "14" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config14.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "15" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config15.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "16" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config16.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "17" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config17.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "18" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config18.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "19" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config19.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "20" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config20.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "21" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config21.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "22" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config22.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "23" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config23.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "24" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config24.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "25" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config25.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "26" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config26.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "27" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config27.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "28" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config28.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "29" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config29.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            ElseIf LK_CODCIA = "30" Then
               Set ArchivoDPA = obj_FSO.CreateTextFile(Leer_Ini(App.Path & "\config30.ini", "CARPETA", "C:\") + sARCHIVOdpa, True)
            Else
            End If
