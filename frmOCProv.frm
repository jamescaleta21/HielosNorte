...
Begin VB.Form frmOCProv 
   Caption         =   "Ingreso de Orden de Compra a Proveedores"
   ClientHeight    =   7185
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11880
   ControlBox      =   0   'False
   Icon            =   "frmOCProv.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ScaleHeight     =   7185
   ...
   Begin VB.CommandButton cmdLoadRerserva 
      Caption         =   "Cargar &Reservas"
      Enabled         =   0   'False
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
   ...
   Begin VB.Frame f1 
      ...
      Begin VB.ComboBox moneda 
         Height          =   315
         ItemData        =   "frmOCProv.frx":000C
         Left            =   3300
         List            =   "frmOCProv.frx":0016
         Style           =   2  'Dropdown List
         TabIndex        =   3
         Tag             =   "cls"
         Top             =   750
      ...
      Begin VB.ComboBox i_fbg 
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "frmOCProv.frx":002F
         Left            =   5805
         List            =   "frmOCProv.frx":0039
         Style           =   2  'Dropdown List
         TabIndex        =   11
         Top             =   2385
         Width           =   1110
      ...
      Begin VB.ComboBox i_destino 
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "frmOCProv.frx":0043
         Left            =   240
         List            =   "frmOCProv.frx":004D
         Style           =   2  'Dropdown List
         TabIndex        =   10
         Top             =   2370
         Width           =   5490
      ...
      Begin VB.ComboBox i_condi 
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "frmOCProv.frx":0066
         Left            =   7905
         List            =   "frmOCProv.frx":0068
         Style           =   2  'Dropdown List
         TabIndex        =   9
         Top             =   2430
         Width           =   2730
      ...
      Begin VB.ComboBox cmdtipo 
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "frmOCProv.frx":006A
         Left            =   8445
         List            =   "frmOCProv.frx":0074
         Style           =   2  'Dropdown List
         TabIndex        =   8
         Top             =   2040
         Width           =   2745
      ...
      Begin VB.Label lcodart 
         AutoSize        =   -1  'True
         Caption         =   "Observaciones : <para pasar a otra fila presione Ctrl +Enter>"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Dim FlagNumOC  As Integer

Dim PSAcuOCProvSol As rdoQuery
Dim RSAcuOCProvSol As rdoResultset
...
Dim vbresp As Integer
    ...
    If vbresp = vbYes Then
        SQL = "UPDATE PEDIDOS SET PED_ESTADO = 'E' WHERE PED_TIPMOV = 500 AND PED_CODCIA='" & LK_CODCIA & "' AND PED_NUMSER='" & txtNumSer.Text & " AND PED_NUMFAC = " & txtNumFac.Text
        CN.Execute SQL
...
On Error GoTo Handler
    ...
    If cmdIngresar.Caption = "&Ingresar" Then
        ...
        f1.Enabled = True
        grd.Enabled = True
        cmdLoadRerserva.Enabled = True
        txtcli.Enabled = True
        txtcli.SetFocus
        ClearForm Me
        FormatGrid
        stbSubtotales.Panels(1).Text = "0.00"
        stbSubtotales.Panels(2).Text = "0.00"
        stbSubtotales.Panels(3).Text = "0.00"
        stbSubtotales.Panels(4).Text = ""
        stbSubtotales.Panels(5).Text = "0.00"
...
End If
    ...
    ped_llave("Ped_DESCTO") = dDouble(grd.TextMatrix(fila, 19)) 'PORCENTAJE
    ped_llave("Ped_DESCTO_PRE") = dDouble(grd.TextMatrix(fila, 18)) 'VALOR
    ped_llave("PED_NUMDOC") = "" 'txtNroDoc.Text
    'ped_llave("Ped_CANATEN") = dDouble(grd.TextMatrix(fila, 16))
    ped_llave("Ped_NUMSER_C") = "0"
    ped_llave("PED_NUMFAC_C") = Val("0") 'Val(txtNumSol.Text)
End Sub

Private Sub cmdLoadRerserva_Click()
    If moneda.ListIndex = -1 Then
        MsgBox "Seleccione el tipo de moneda!!!", vbInformation, Pub_Titulo
        moneda.SetFocus
        Exit Sub
    End If
    LoadReserva
...
On Error GoTo ErroHandle
    sSql = "SELECT 'Razon Social de la Empresa'=cli_nombre, 'Codigo'=cli_codcliE, 'RUC'=CLI_RUC_ESPOSO, 'Direccion'=cli_casa_direc FROM CLIENTES WHERE CLI_CP='P' AND CLI_NOMBRE LIKE '" & oKeyFind & "%' AND CLI_CODCIA = '" & LK_CODCIA & "' ORDER BY CLI_NOMBRE"
...
Private Sub FormatGrid()
    ...
    With grd
        ...
        .Height = 4530
        .Cols = 20
        .FormatString = "Descripción|Código|Cantidad|Unidad|Precio||||||SubTotal||||||||Desc|Descto%"
        .ColWidth(0) = 3800 'descripcion
        .ColWidth(1) = 1200 'codigo
        .ColWidth(2) = 900 'cantidad
        .ColWidth(3) = 1000 'unidad
        .ColWidth(4) = 900 'precio
        .ColWidth(5) = 0 'laboiratorio
        .ColWidth(6) = 0  'marca
        ...
        .ColWidth(17) = 0 'CANTIDAD acumulada de OC a Proveedores que todavia no son atendidas
        .ColWidth(18) = 1000  ' descuento
        .ColWidth(19) = 1200 ' % descuento
...
Dim iRows As Integer
    ...
    grd.TextMatrix(RowA, 10) = Format(PUB_SUBTOTAL, "##0.00")
    'porcentaje
    PUB_DESCTO = PUB_SUBTOTAL * Val(grd.TextMatrix(RowA, 19)) / 100
    grd.TextMatrix(RowA, 18) = Format(PUB_DESCTO, "##0.00")
    ...
    For iRow = 1 To iRows - 1
        PUB_DESCTO = Val(grd.TextMatrix(iRow, 18)) + PUB_DESCTO
        PUB_SUBTOTAL = dDouble(grd.TextMatrix(iRow, 10)) + PUB_SUBTOTAL
    Next iRow
    ...
    stbSubtotales.Panels(1).Text = Format(PUB_SUBTOTAL, "##0.00")
    stbSubtotales.Panels(2).Text = Format(PUB_DESCTO, "##0.00")
    stbSubtotales.Panels(3).Text = Format(PUB_IMPTO, "##0.00")
    stbSubtotales.Panels(5).Text = Format(PUB_IMPORTE, "##0.00")
End Sub
...
Private Sub NumOC()
    ...
    archi = "SELECT MAX(PED_NUMFAC) AS NUMFAC FROM PEDIDOS WHERE PED_TIPMOV=500 AND Ped_FBG = 'O' AND PED_CODCIA = '" & LK_CODCIA & "'"
...
Private Sub grd_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        FLAG = 1 'Editar Celda
        If grd.COL = 2 Or grd.COL = 4 Or grd.COL = 19 Then
            txt = ""
            txt.Visible = False
    ...
    ElseIf KeyCode = 45 And grd.COL = 4 Then
        ...
        CalculaTotales1 grd.Row, 4
        ValRowGrd grd.Row
        If ColumnNext(grd.COL) = -1 And Trim(grd.TextMatrix(grd.Row, 13)) = "X" Then GoTo OtraFila
...
OtraFila:
                If grd.Rows = 11 Then
                    MsgBox "Se llego al tope de items", vbInformation, Pub_Titulo
                    Exit Sub
                End If
                grd.Rows = grd.Rows + 1
            End If
...
OtraFila:
    ...
    End If
'    If grd.COL = 1 Then
'        KeyAscii = vString(KeyAscii)
'    ElseIf m_pTypeData = 2 Then
'        KeyAscii = vInteger(KeyAscii)
'    ElseIf m_pTypeData = 3 Then
'        KeyAscii = vNumeric(KeyAscii)
'    ElseIf m_pTypeData = 4 Then
'        KeyAscii = KeyAscii
'    End If
    If grd.COL = 4 Or grd.COL = 2 Or grd.COL = 19 Then
        KeyAscii = vNumeric(KeyAscii)
    End If
    '=========
'    eString = 1
...
On Error Resume Next
    ...
    lvw.ColumnHeaders.Add , "Laboratorio", "Laboratorio", 2000, lvwColumnLeft
...
Private Function ConsisGrl() As Integer
    If Consis5(txtcli.TEXTO) = 1 Then
        MsgBox "Proveedor no existe....", 46, Pub_Titulo
        ConsisGrl = 1
        txtcli.SetFoco
        Exit Function
    End If
...
'    End If
    ...
    If Consis4(txtFecha.Text) = 1 Then
        ...
        Exit Function
    End If
    If consis2(moneda.Text) = 1 Then
        MsgBox "Falta seleccionar la moneda....", 46, Pub_Titulo
        ConsisGrl = 1
        moneda.SetFocus
        Exit Function
    End If
...
Private Sub LoadReserva()
Dim iSec As Integer
Dim AcuSolP  As Integer
Dim AcuSol  As Integer
Dim Equival As Integer

    archi = "SELECT * FROM vw_artfaloc WHERE PED_CODCIA='" & LK_CODCIA & "'"
    Set PSX = CN.CreateQuery("", archi)
    Set X = PSX.OpenResultset(rdOpenKeyset)
    X.Requery
    If Not X.EOF Then
        ...
        Do While Not X.EOF
            ...
            'LOAD ARTICULO
            SQ_OPER = 1
            PUB_CODART = X("Ped_CODART")
            pu_codcia = LK_CODCIA
            LEER_ARM_LLAVE
            If Not arm_llave.EOF Then
                If arm_llave("arm_stock") - X("Ped_CANTIDAD") < 0 Then
                    iSec = iSec + 1
                    grd.Rows = grd.Rows + 1
                    grd.TextMatrix(iSec, 2) = Format(X("Ped_CANTIDAD") - arm_llave("arm_stock"), "#0.000")
                Else
                    GoTo SEGUIR
                End If
            Else
            ...
            pu_codcia = LK_CODCIA
            PUB_CODART = X("Ped_CODART")
            PUB_UNIDADS = Trim(X("PRE_UNIDAD"))
            LEER_PRE_LLAVE
            If Not pre_unidad.EOF Then
                grd.TextMatrix(iSec, 11) = pre_unidad("Pre_Equiv")
                If PUB_DS = "S" Then
                    grd.TextMatrix(iSec, 4) = pre_unidad("Pre_costo")
                ElseIf PUB_DS = "D" Then
                    grd.TextMatrix(iSec, 4) = Format(pre_unidad("Pre_costo") / LK_IGV, "0.000")
                End If
                Equival = pre_unidad("Pre_Equiv")
            End If
            'LOAD ARTI
            SQ_OPER = 1
            PUB_KEY = X("Ped_CODART")
            pu_codcia = LK_CODCIA
            LEER_ART_LLAVE
            ...
            Else
                MsgBox "Error grave en Arti...", 46, Pub_Titulo
            End If
            'LOAD ACUMULADO DE ORDEN DE COMPRA A PROVEEDORES EN ESPERA
            archi = "SELECT * FROM VW_AcuOCProvPend WHERE PED_CODCIA = '" & LK_CODCIA & "' AND PED_CODART = " & X("Ped_CODART")
            Set PSAcuOCProvSol = CN.CreateQuery("", archi)
            Set RSAcuOCProvSol = PSAcuOCProvSol.OpenResultset(rdOpenKeyset)
            RSAcuOCProvSol.Requery
            If Not RSAcuOCProvSol.EOF Then
                grd.TextMatrix(iSec, 17) = RSAcuOCProvSol("Ped_Cantidad")
                BackColorRow iSec, grd, &H80000018
            End If
SEGUIR:
    ...
    Else
        FlagSolicitud = 3
        MsgBox "No existen reservas...!!!!", vbInformation, Pub_Titulo
        cmdcancelar_Click
    End If
End Sub
Private Sub LoadOC()
...
'        stbSubtotales.Panels(5).Text = Format(PUB_IMPORTE, "##0.000")
        ...
        '    ped_llave("Ped_ESTADO") = "N"
            ...
            grd.TextMatrix(iSec, 15) = ped_llave("Ped_NUMPRE")
            grd.TextMatrix(iSec, 16) = ped_llave("Ped_CANTIDAD")
            grd.TextMatrix(iSec, 18) = ped_llave("Ped_DESCTO_PRE")
            grd.TextMatrix(iSec, 19) = ped_llave("Ped_DESCTO")
            'PUB_DESCTO = PUB_DESCTO + ped_llave("Ped_DESCTO_PRE")
            SQ_OPER = 1
        ...
        If CountItemA = iSec Then
            FlagSolicitud = 3
            MsgBox "Orden esta atendida Totalmete", vbInformation, Pub_Titulo
            Exit Sub
        End If
...
'                grd.TextMatrix(grd.Row, 4) = pre_llave("PRE_PRE5")
'            End If
            grd.TextMatrix(grd.Row, 4) = Nulo_Valor0(pre_llave("Pre_Costo"))
        ElseIf PUB_DS = "D" Then
...
'                grd.TextMatrix(grd.Row, 4) = pre_llave("PRE_PRE55")
'            End If
            grd.TextMatrix(grd.Row, 4) = Format(Nulo_Valor0(pre_llave("Pre_Costo")) * LK_TIPO_CAMBIO, "0.0000")
        End If
    Else
        MsgBox "Error grave en precios", 46, Pub_Titulo
    End If
...
Dim DIF As Integer
    ...
    CanOC = dDouble(grd.TextMatrix(lRow, 2))
    CodArt = grd.TextMatrix(lRow, 12)
    archi = "SELECT SUM(PED_CANTIDAD) as SumCant FROM PEDIDOS WHERE PED_SITUACION=' ' AND PED_TIPMOV=400 AND PED_CODART = " & CodArt & " AND PED_CODCIA = '" & LK_CODCIA & "'"
    Set PSX = CN.CreateQuery("", archi)
    Set X = PSX.OpenResultset(rdOpenKeyset)
    X.Requery
    If X.EOF Then
...
On Error GoTo Handler
    ...
    Reportes.ReportFileName = PUB_RUTA_OTRO & "OCPROV.RPT"
    Reportes.Formulas(0) = "CIA='" & Trim(par_llave("par_nombre")) & " RUC:" & par_llave("par_nombre_corto") & "'"
    Reportes.WindowTitle = "Orden de Compra a Proveedor"
    Reportes.Destination = crptToWindow
    Reportes.SelectionFormula = " {PEDIDOS.PED_NUMFAC} = " & txtNumFac.Text & " AND {PEDIDOS.PED_NUMSER} = '" & txtNumSer.Text & "' AND {PEDIDOS.PED_CODCIA} = '" & LK_CODCIA & "' AND {PEDIDOS.PED_TIPMOV} = 500 AND {PEDIDOS.PED_ESTADO} <> 'E'"
    Reportes.Action = 1
    Exit Sub
Handler:
    MsgBox Err.Description
