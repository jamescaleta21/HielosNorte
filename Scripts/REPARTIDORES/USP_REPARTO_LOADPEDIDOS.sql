...
USP_REPARTO_LOADPEDIDOS '20241030',5,'01',1
*/
...
SELECT p.idpedido,
	   ...
	   'SUBRUTA ' + CAST(COALESCE(p.IDSUBRUTA,0) AS VARCHAR(10)) AS 'subruta'
...
WHERE p.fecha = @FECHA
	  ...
	  AND (COALESCE(p.IDSUBRUTA,0) = @SUBRUTA OR @SUBRUTA = -1)
	  GROUP BY c.CLI_NOMBRE,p.idpedido,p.observacion,COALESCE(p.IDSUBRUTA,0)
	  ;
